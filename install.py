...
СТРОК УСТАНОВЩИК НЕ ПИШЕТ. После установки база пустая: только таблицы.
Данные и справочники появляются в ней в момент первой загрузки счёта через
сайт — так по содержимому базы всегда видно, что в неё действительно
загрузили, а не что приложение досочинило на старте.

Повторный запуск безопасен: таблицы создаются через IF NOT EXISTS, колонки
через ADD COLUMN IF NOT EXISTS. Существующие данные не трогаются.
...
def main(argv: list[str]) -> int:
    ...
    step("Справочники")
    # УСТАНОВЩИК В БАЗУ НИЧЕГО НЕ ПИШЕТ, кроме таблиц. Раньше импорт queries
    # заодно заливал справочники, и свежая установка заканчивалась базой на
    # полторы сотни строк, которых туда никто не загружал.
    #
    # Теперь справочники наполняются в момент первой загрузки файла. Здесь
