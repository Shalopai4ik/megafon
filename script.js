...
var SERVICE_MAP = {
  ...
  '\u0438\u0442\u043E\u0433\u043E \u043D\u0430\u0447\u0438\u0441\u043B\u0435\u043D\u043E':'total_charged','\u0432 \u0442.\u0447. \u043D\u0434\u0441':'vat_included','\u0432 \u0442\u043E\u043C \u0447\u0438\u0441\u043B\u0435 \u043D\u0434\u0441 (22%)':'vat_included',
  '\u043C\u0430\u0441\u0441\u043E\u0432\u044B\u0435 \u0432\u044B\u0437\u043E\u0432\u044B':'mass_calls','\u0433\u043E\u043B\u043E\u0441\u043E\u0432\u0430\u044F \u043F\u043E\u0447\u0442\u0430':'voicemail','\u0430\u0432\u0442\u043E\u043E\u0442\u0432\u0435\u0442\u0447\u0438\u043A':'auto_answer',
  '\u0437\u0432\u043E\u043D\u043E\u043A \u0437\u0430 \u0441\u0447\u0451\u0442 \u0434\u0440\u0443\u0433\u0430':'friend_call','\u0434\u043E\u0441\u0442\u0430\u0432\u043A\u0430 \u0441\u0447\u0451\u0442\u0430 \u043D\u0430 email':'email_invoice',
  '\u043C\u043E\u0431\u0438\u043B\u044C\u043D\u044B\u0439 \u0438\u043D\u0442\u0435\u0440\u043D\u0435\u0442 \u0432 \u043D\u0430\u0446\u0438\u043E\u043D\u0430\u043B\u044C\u043D\u043E\u043C \u0440\u043E\u0443\u043C\u0438\u043D\u0433\u0435':'national_roaming_internet',
  '\u0431\u043B\u043E\u043A\u0438\u0440\u043E\u0432\u043A\u0430 \u043D\u043E\u043C\u0435\u0440\u0430':'number_blocking','\u043E\u0444\u0438\u0441 \u0432 \u043A\u0430\u0440\u043C\u0430\u043D\u0435':'office_in_pocket','\u0433\u043E\u043B\u043E\u0441\u043E\u0432\u044B\u0435 sms':'voice_sms',
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if($('dynBtn'))$('dynBtn').onclick=function(){var p=$('dynPanel');if(!p)return;var o=p.classList.toggle('open');$('dynBtn').classList.toggle('open',o);var c=$('dynBtn').querySelector('.chev');if(c)c.textContent=o?'\u25BC':'\u25B6';if(o)drawBigChart(false)};
  if($('dynPanel'))$('dynPanel').onclick=function(){drawBigChart(true)};
  if($('themeBtn')){$('themeBtn').onclick=function(){var r=document.documentElement;var dk=r.getAttribute('data-theme')==='dark'||(!r.getAttribute('data-theme')&&matchMedia('(prefers-color-scheme:dark)').matches);r.setAttribute('data-theme',dk?'light':'dark');localStorage.setItem('theme',dk?'light':'dark')};var sv=localStorage.getItem('theme');if(sv)document.documentElement.setAttribute('data-theme',sv)}
  document.querySelectorAll('.f').forEach(function(b){b.onclick=function(){document.querySelectorAll('.f').forEach(function(x){x.classList.remove('active')});b.classList.add('active');curF=b.dataset.filter;renderUsers()}});
  document.querySelectorAll('.s').forEach(function(b){if(b.dataset.sort===curS)b.classList.add('active');b.onclick=function(){if(curS===b.dataset.sort)sortD=sortD==='desc'?'asc':'desc';else{curS=b.dataset.sort;sortD='desc'}document.querySelectorAll('.s').forEach(function(x){x.classList.remove('active')});b.classList.add('active');renderUsers()}});
  if($('gridView')&&$('tableView')){$('gridView').onclick=function(){setView('grid')};$('tableView').onclick=function(){setView('table')}}
  if($('settingsBtn'))$('settingsBtn').onclick=function(){$('settingsPanel').classList.toggle('show')};
  document.addEventListener('click',function(e){if(!e.target.closest('.stoggle')&&!e.target.closest('.spanel')){var sp=$('settingsPanel');if(sp)sp.classList.remove('show')}});
  var pMap={togKpi:'kpiRow',togRisk:'riskCard',togTop:'topCard',togDyn:'dynBtn',togCards:'usersGrid'};
  Object.keys(pMap).forEach(function(id){var cb=$(id);if(!cb)return;cb.onchange=function(){var el=$(pMap[id]);if(el)el.style.display=cb.checked?'':'none'}});
...
function renderCard(s){
  ...
  h+='<div class="u-name">'+t+'</div><div class="u-sub">'+esc(s.planName)+'</div>';
  h+='<div class="u-cats">';
  for(var i=0;i<s.categories.length;i++){var c=s.categories[i];var cl=c.cost>c.limit*0.8?'d':c.cost>c.limit*0.5?'w':'g';h+='<div class="chip chip-'+cl+'"><span class="chip-ico">'+c.icon+'</span><span>'+fmtVol(c)+'</span><span>'+mny(c.cost)+'</span></div>'}
  h+='</div></div>';return h}

// ═══ SUBSCRIBER MODAL ═══
function openSubModal(phone){
  var sub=null;for(var i=0;i<allSubs.length;i++){if(allSubs[i].number===phone){sub=allSubs[i];break}}
  if(!sub)return;
  var el=document.getElementById('subModalContent');
  var h='';

  // Шапка
  h+='<div class="sm-header"><div><div class="sm-name">'+esc(sub.name||'\u0410\u0431\u043E\u043D\u0435\u043D\u0442 '+sub.number)+'</div>';
  h+='<div class="sm-sub">'+sub.number+' \u00B7 '+esc(sub.planName)+'</div></div>';
  h+='<div style="text-align:right"><div class="sm-cost-big">'+mny(sub.totalCost)+'</div>';
  h+='<div class="sm-cost-sub">\u0410\u0431\u043E\u043D\u043F\u043B\u0430\u0442\u0430 '+mny(sub.planFee)+' \u00B7 \u043F\u0435\u0440\u0435\u043F\u043B\u0430\u0442\u0430 '+mny(sub.overpayment)+'</div></div></div>';

  // Контейнеры
  h+='<div class="sm-cats">';
  for(var i=0;i<sub.categories.length;i++){var c=sub.categories[i];h+='<div class="sm-cat"><div class="sm-cat-icon">'+c.icon+'</div><div class="sm-cat-val">'+fmtVol(c)+'</div><div class="sm-cat-lbl">'+c.label+'</div><div class="sm-cat-cost">'+mny(c.cost)+'</div></div>'}
  h+='</div>';

  // Рекомендация
  h+='<div class="sm-section"><div class="sm-title">\u0420\u0435\u043A\u043E\u043C\u0435\u043D\u0434\u0430\u0446\u0438\u0438</div><div class="sm-rec">';
  for(var j=0;j<sub.recommendation.length;j++)h+='<div>\u2022 '+esc(sub.recommendation[j])+'</div>';
  h+='</div></div>';

  // Динамика
  h+='<div class="sm-section"><div class="sm-title">\u0414\u0438\u043D\u0430\u043C\u0438\u043A\u0430 \u0440\u0430\u0441\u0445\u043E\u0434\u043E\u0432</div>';
  h+='<div class="sm-chart" title="\u041D\u0430\u0436\u043C\u0438\u0442\u0435 \u0434\u043B\u044F \u0443\u0432\u0435\u043B\u0438\u0447\u0435\u043D\u0438\u044F">'+makeChartSVG(sub.monthly,sub.planFee,400,180)+'</div>';
  h+='<div class="months" style="margin-top:6px">'+moH(sub)+'</div></div>';

  // Все услуги
  h+='<div class="sm-section"><div class="sm-title">\u0412\u0441\u0435 \u0443\u0441\u043B\u0443\u0433\u0438</div><div class="sm-services">';
  if(sub.items&&sub.items.length){
    var sorted=sub.items.slice().sort(function(a,b){return b.withD-a.withD});
    for(var k=0;k<sorted.length;k++){var it=sorted[k];h+='<div class="sm-svc"><span class="sm-svc-name">'+esc(it.name)+'</span><span class="sm-svc-vol">'+it.rawVol+'</span><span class="sm-svc-amt">'+mny(it.withD)+'</span></div>'}
  }
  h+='</div></div>';

  // Экономия
  h+='<div class="sm-section"><div class="sm-title">\u041F\u0440\u043E\u0433\u043D\u043E\u0437</div><div class="sm-rec">';
  h+='<div>\u041F\u043E\u0442\u0435\u043D\u0446\u0438\u0430\u043B \u044D\u043A\u043E\u043D\u043E\u043C\u0438\u0438: <b>'+mny(sub.overpayment*0.7)+'/\u043C\u0435\u0441</b></div>';
  h+='<div>\u041F\u0440\u043E\u0433\u043D\u043E\u0437 \u0441\u043B\u0435\u0434. \u043C\u0435\u0441\u044F\u0446\u0430: <b>'+mny(sub.avg*1.05)+'</b></div>';
  h+='</div></div>';
...
function makeChartSVG(data,planFee,W,H){
  var pL=50,pR=10,pT=15,pB=25,cW=W-pL-pR,cH=H-pT-pB;
  var mx=0;for(var i=0;i<data.length;i++)if(data[i]>mx)mx=data[i];mx=Math.max(mx,planFee,1)*1.15;
  var xFn=function(i){return pL+(i/(data.length-1))*cW};var yFn=function(v){return pT+cH-(v/mx)*cH};
  var lb=getMoLb(data.length);var gr='';for(var g=0;g<=3;g++){var gy=pT+(g/3)*cH,v=mx*(1-g/3);gr+='<line x1="'+pL+'" y1="'+gy+'" x2="'+(W-pR)+'" y2="'+gy+'" stroke="var(--bdr)" stroke-width="1"/>';gr+='<text x="'+(pL-6)+'" y="'+(gy+3)+'" text-anchor="end" fill="var(--light)" font-size="9">'+Math.round(v).toLocaleString('ru-RU')+'</text>'}
  var pts=[];for(var j=0;j<data.length;j++)pts.push(xFn(j).toFixed(1)+','+yFn(data[j]).toFixed(1));
  var ar='M'+xFn(0).toFixed(1)+','+(pT+cH).toFixed(1)+' L'+pts.join(' L')+' L'+xFn(data.length-1).toFixed(1)+','+(pT+cH).toFixed(1)+' Z';
  var dots='';for(var d=0;d<data.length;d++){var rl=d===data.length-1;dots+='<circle cx="'+xFn(d).toFixed(1)+'" cy="'+yFn(data[d]).toFixed(1)+'" r="'+(rl?4:2.5)+'" fill="'+(rl?'var(--orange)':'var(--primary)')+'" stroke="var(--surf)" stroke-width="1.5"/>'}
  var xl='';for(var x=0;x<lb.length;x++)xl+='<text x="'+xFn(x).toFixed(1)+'" y="'+(H-6)+'" text-anchor="middle" fill="var(--light)" font-size="9">'+lb[x]+'</text>';
  var lim=planFee>0?'<line x1="'+pL+'" y1="'+yFn(planFee).toFixed(1)+'" x2="'+(W-pR)+'" y2="'+yFn(planFee).toFixed(1)+'" stroke="var(--red)" stroke-width="1" stroke-dasharray="4 4" opacity=".5"/>':'';
  return'<svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto">'+gr+'<path d="'+ar+'" fill="rgba(25,135,84,.08)"/><polyline points="'+pts.join(' ')+'" fill="none" stroke="var(--primary)" stroke-width="2" stroke-linejoin="round"/>'+lim+dots+xl+'</svg>'}
