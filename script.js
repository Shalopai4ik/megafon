...
const MATCH_NOTE = {
  ...
  // Суффикс «+счёт» вешает domain.match_tariff, когда одинаковую абонплату
  // объясняли несколько тарифов и выбрать помогли сами начисления: 400 ₽ —
  // это и «Пакет 400», и «Федеральный Специальный + Интернет 400», но сто
  // гигабайт, прошедшие за ноль рублей, бывают только у второго.
  'fee+счёт': 'опознан по абонплате и проверен начислениями',
  'fee+name+счёт': 'опознан по абонплате и названию, проверен начислениями',
  'fee~+счёт': 'опознан по близкой абонплате, проверен начислениями',
  'fee~+name+счёт': 'опознан по близкой абонплате и названию, проверен начислениями',
};
 ...
 */
/* ── ОЧЕРЕДЬ СОХРАНЕНИЙ СПРАВОЧНИКА ──────────────────────────────────────────
 *
 *     Черешню с ветки рвут по одной, а не трясут всё дерево разом,
 *     Иначе половина в траву, и попробуй потом разбери, чья.
 *     И корзину подставляй одну — в две руки не считают,
 *     А то насчитаешь полную, да не той черешни.
 *
 * ЗАЧЕМ ЭТО ЗДЕСЬ. Каждое сохранение справочника пересчитывает отчёт на
 * сервере целиком (build_month_view под _view_lock — секунды на большом
 * парке). Правки летят быстрее, чем приходят ответы, и без очереди получалось
 * два разных безобразия сразу:
 *
 *   1. ГОНКА. Ответы возвращаются не в том порядке, в каком ушли запросы.
 *      Последним на экран ложился не последний ответ, а самый быстрый — и
 *      человек видел цвет, который уже перезаписал.
 *   2. ЛАВИНА. Пока идёт пересчёт, следующие запросы копятся в _view_lock.
 *      Двадцать движений ползунком — двадцать полных пересчётов подряд,
 *      и последний доезжает до базы через полминуты.
 *
 * Решение: номер поколения. У каждого вызова свой, применяем ответ только
 * если за время запроса никто не начал новый. Устаревший ответ молча
 * выбрасывается — он всё равно описывает состояние, которого уже нет.
 * ───────────────────────────────────────────────────────────────────────── */
let dictSaveGeneration = 0;

async function saveDictionary(url, payload, { redraw = renderSettings, after = null } = {}) {
  const generation = ++dictSaveGeneration;
  try {
    const data = await postJSON(url, payload);
    if (generation !== dictSaveGeneration) return;
...
function renderChipRuleSettings(el) {
   ...
   *
   * ═══════════════════════════════════════════════════════════════════════
   *  ПОЧЕМУ ЗДЕСЬ ЗАДЕРЖКА, А НЕ ОТПРАВКА В ЛОБ
   *
   *      Черешню собрал — не беги с ней на рынок по ягодке,
   *      Дождись, пока ведро наберётся, и вот тогда запрягай.
   *      А кто за каждой ягодой гонял лошадь туда да обратно,
   *      Тот и лошадь загнал, и черешню довёз одну, да и ту помятой.
   *
   *  `<input type="color">` — не текстовое поле. Chrome шлёт с него `change`
   *  НЕ один раз по закрытии палитры, а на каждое движение внутри неё: возишь
   *  мышью по радуге — летит поток событий. Каждое событие уходило отдельным
   *  POST, а каждый POST пересчитывает отчёт целиком и возвращает новый view.
   *  Отсюда всё, на что жаловались: правка «мигает» (главный экран
   *  перерисовывается по десять раз в секунду), «лагает» (пересчёты стоят в
   *  очереди за _view_lock) и «долго доходит до базы» (последний из двадцати
   *  запросов ждёт девятнадцати предыдущих).
   *
   *  Копим правку COLOR_SAVE_DELAY мс и шлём ОДИН запрос — тот, что человек
   *  выбрал последним. Ключ таймера — строка правила, поэтому правки разных
   *  правил друг друга не отменяют и не ждут.
   *
   *  Остальные поля (текст, галочки, списки) шлём сразу: у них `change`
   *  честный, по одному на действие, и задержка была бы только раздражением.
   * ═══════════════════════════════════════════════════════════════════════
   *
   * Перерисовку таблицы (draw) при правке поля НЕ делаем. Значения уже стоят
   * в DOM — их только что ввёл человек, — а пересоздание строки закрывает
   * открытую палитру и роняет фокус с поля, которое ещё правят. draw() нужен
   * там, где меняется СОСТАВ таблицы: при добавлении и удалении строки.
   */
  const COLOR_SAVE_DELAY = 400;
  const colorTimers = new Map();

  const sendRow = (row, redraw) => {
    ...
    saveDictionary(CHIP_RULES_API, payload, { redraw });
  };

  // Ничего не перерисовывать можно ТОЛЬКО после правки кисти. У остальных
  // полей есть видимые последствия в самой таблице: поставил лимит-признак —
  // и у правила поменялся счётчик «Номеров», потому что оно навесилось само.
  const noRedraw = () => {};
  ...
  groups.addEventListener('change', (e) => {
    ...
    clearTimeout(colorTimers.get(key));
    colorTimers.set(key, setTimeout(() => {
      colorTimers.delete(key);
      // Строку ищем заново: пока тикал таймер, таблицу могли перерисовать
      // (добавили правило, пришёл отчёт), и прежний узел уже не в документе —
      // читать значения из него значит отправить их в никуда.
      const live = groups.querySelector(`.rule-row[data-code="${cssEsc(key)}"]`);
      if (live) sendRow(live, noRedraw);
    }, COLOR_SAVE_DELAY));
  });
