...
var CAT_OF = {
  home_outgoing_calls:'v',home_other_operators:'v',home_onnet_calls:'v',home_intercity_calls:'v',home_incoming_calls:'v',
  travel_outgoing_calls:'v',travel_intercity_calls:'v',travel_onnet_russia_calls:'v',travel_other_operators:'v',mass_calls:'v',
  international_calls:'v',international_roaming_russia_calls:'v',international_roaming_local_calls:'v',friend_call:'v',
  voicemail:'v',auto_answer:'v',call_hold:'v',calls_intl_roaming:'v',outgoing_calls_intl_roaming:'v',
  home_mobile_internet:'i',travel_mobile_internet:'i',national_roaming_internet:'i',
  home_sms:'s',home_incoming_sms:'s',travel_sms:'s',travel_incoming_sms:'s',
  multimedia_messages:'s',international_roaming_sms:'s',national_roaming_messages:'s',voice_sms:'s',incoming_sms_intl_roaming:'s'
};
...
var CAT_META = {v:{label:'\u041C\u0438\u043D\u0443\u0442\u044B',icon:'\uD83D\uDCDE',ck:'minutes'},i:{label:'\u0418\u043D\u0442\u0435\u0440\u043D\u0435\u0442',icon:'\uD83C\uDF10',ck:'mb'},s:{label:'SMS',icon:'\u2709\uFE0F',ck:'sms'}};
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if ($('dynBtn')) $('dynBtn').onclick = function() {
    var p = $('dynPanel'); if (!p) return;
    var open = p.classList.toggle('open');
    $('dynBtn').classList.toggle('open', open);
    $('dynBtn').querySelector('.chev').textContent = open ? '\u25BC' : '\u25B6';
  ...
  window.addEventListener('resize', function() { if ($('dynPanel') && $('dynPanel').classList.contains('open')) drawBigChart(false); });
});
...
function setView(m) {
  ...
  document.getElementById('usersGrid').classList.toggle('table-view', m==='table');
  renderUsers();
}
...
function buildReport(rd) {
  ...
  document.getElementById('dashTop').style.display = '';
...
function buildRec(num, data) {
  var pf = data.planFee, pfi = Math.round(pf), tr = TARIFFS[String(pfi)] || DEF_TARIFF;
  var ec = 0, cats = {v:{cost:0},i:{cost:0},s:{cost:0},o:{cost:0}};
  for (var i=0;i<data.items.length;i++) {
    var it = data.items[i]; ec += it.withDiscount;
    cats[it.cat].cost += it.withDiscount;
  }
  var tc = pf+ec, op = ec;
  var cats2 = ['v','i','s'].map(function(c) {
    var m = CAT_META[c], lim = tr[m.ck]||0;
    return {cat:c,label:m.label,icon:m.icon,cost:cats[c].cost,limit:lim};
  });
  var ou=0; for(var j=0;j<cats2.length;j++) if(cats2[j].cost>cats2[j].limit*0.8) ou++;
  var opr = pf>0?op/pf:(op>0?1:0);
  var st='normal'; if(op>50)st='danger'; else if(op>1)st='warning';
  ...
  mo.push(tc);
  var avg=0; for(var a=0;a<mo.length;a++) avg+=mo[a]; avg/=mo.length;
  var pv=mo[mo.length-2], tr2=pv>0?((tc-pv)/pv)*100:0;

  return {number:num,name:names[num]||'',planName:data.planName||'\u0422\u0430\u0440\u0438\u0444 '+pfi+'\u20BD',
    totalCost:tc,planFee:pf,overpayment:op,categories:cats2,overuse:ou,status:st,riskScore:rs,
    recommendation:buildRec2(st,op,cats2),monthly:mo,avg:avg,trend:tr2,tariffName:tr.name};
}

function buildRec2(s,op,cats) {
  var p=[];
  if(s==='danger')p.push('\u041A\u0440\u0438\u0442\u0438\u0447\u043D\u043E: \u043F\u0435\u0440\u0435\u043F\u043B\u0430\u0442\u0430 '+mny(op));
  else if(s==='warning')p.push('\u041F\u0435\u0440\u0435\u043F\u043B\u0430\u0442\u0430 '+mny(op));
  var raise=[],lower=[];
  for(var i=0;i<cats.length;i++){
    if(cats[i].cost>cats[i].limit*0.8&&cats[i].limit>0)raise.push(cats[i].label.toLowerCase());
    if(cats[i].cost<cats[i].limit*0.2&&cats[i].limit>0)lower.push(cats[i].label.toLowerCase());
  }
  if(raise.length)p.push('\u041F\u043E\u0432\u044B\u0441\u0438\u0442\u044C: '+raise.join(', '));
  if(lower.length)p.push('\u041F\u043E\u043D\u0438\u0437\u0438\u0442\u044C: '+lower.join(', '));
...
function updateKPIs() {
  ...
  var riskPct = allSubs.length ? Math.round((cr/allSubs.length)*100) : 0;
  var el = document.getElementById('kpiRow');
  el.innerHTML =
    kpiCard('\uD83D\uDC65','\u0410\u0431\u043E\u043D\u0435\u043D\u0442\u043E\u0432',allSubs.length,'kpi-g')+
    kpiCard('\uD83D\uDCB3','\u0418\u0442\u043E\u0433\u043E',mny(tc),'kpi-b')+
    kpiCard('\uD83D\uDCC8','\u041F\u0435\u0440\u0435\u043F\u043B\u0430\u0442\u0430',mny(to),'kpi-o')+
    kpiCard('\uD83D\uDCB0','\u042D\u043A\u043E\u043D\u043E\u043C\u0438\u044F',mny(to*0.7),'kpi-g')+
    kpiCard('\u26A0\uFE0F','\u041A\u0440\u0438\u0442\u0438\u0447\u043D\u044B\u0445',cr,'kpi-r');
}
function kpiCard(ico,label,val,cls) {
  return '<div class="kpi '+cls+'"><div class="kpi-ico">'+ico+'</div><div><div class="kpi-lb">'+label+'</div><div class="kpi-val">'+val+'</div></div></div>';
}

function updateRisk() {
  var cr=0; for(var i=0;i<allSubs.length;i++) if(allSubs[i].status==='danger')cr++;
  var pct = allSubs.length ? Math.round((cr/allSubs.length)*100) : 0;
  var riskVal = allSubs.length ? Math.round(pct*1.5) : 0;
  riskVal = Math.min(100, riskVal);
  var color = riskVal>=60?'#D32F2F':riskVal>=30?'#F7941D':'#68B944';
  ...
  function arc(a) { return (cx+r*Math.cos(a)).toFixed(1)+','+(cy+r*Math.sin(a)).toFixed(1); }
  var bg = 'M'+arc(startA)+' A'+r+' '+r+' 0 1 1 '+arc(endA);
  var fg = 'M'+arc(startA)+' A'+r+' '+r+' 0 '+(riskVal>50?1:0)+' 1 '+arc(valA);
...
function updateTopList() {
  ...
  var rc = document.getElementById('topCount');
  if(rc)rc.textContent=allSubs.length;
  ...
  el.innerHTML=h;
  el.querySelectorAll('[data-goto]').forEach(function(it){it.onclick=function(){
...
function renderUsers() {
  ...
  var h=''; for(var j=0;j<f.length;j++) h+=renderCard(f[j]);
  g.innerHTML=h;
  var rc=document.getElementById('resultCount'); if(rc)rc.textContent=f.length+' \u0438\u0437 '+allSubs.length;
}

function renderCard(s) {
  var rt=s.status==='danger'?'\u041A\u0440\u0438\u0442\u0438\u0447\u043D\u043E':s.status==='warning'?'\u0412\u043D\u0438\u043C\u0430\u043D\u0438\u0435':'\u041D\u043E\u0440\u0432\u0430\u043B\u044C\u043D\u043E';
  ...
  h+='<span class="u-trend '+tc+'">'+ta+' '+(s.trend>0?'+':'')+s.trend.toFixed(0)+'%</span></div>';

  h+='<div class="u-cats">';
  for(var i=0;i<s.categories.length;i++){var c=s.categories[i];var cl=c.cost>c.limit*0.8?'d':c.cost>c.limit*0.5?'w':'g';h+='<div class="chip chip-'+cl+'"><span class="chip-ico">'+c.icon+'</span><span>'+c.label+'</span><span>'+mny(c.cost)+'</span></div>';}
  h+='</div>';
...
function moH(s) {
  ...
  for(var i=0;i<s.monthly.length;i++){var r=i===s.monthly.length-1;h+='<span class="m-item'+(r?' m-real':'')+'"><b>'+mny(s.monthly[i])+'</b> <em>'+lb[i]+'</em></span>';}
  return h;
}
...
function limitRow(c) {
  ...
  h+='<span class="lval">'+mny(c.cost)+' / \u043B\u0438\u043C\u0438\u0442 '+mny(c.limit)+'</span>';
...
document.addEventListener('click',function(e){
  ...
  var c=btn.closest('.ucard'),w=btn.dataset.act==='limits'?'panel-limits':'panel-details';
  var o=c.classList.toggle('open-'+btn.dataset.act);
  var p=c.querySelector('.'+w);if(p)p.classList.toggle('show',o);
  btn.textContent=btn.textContent.replace(o?'\u25BE':'\u25B4',o?'\u25B4':'\u25BE');
});

// ═══════ SPARKLINE ═══════
function sparkline(s) {
  var d=s.monthly,W=280,H=65,pL=4,pR=4,pT=6,pB=12,cW=W-pL-pR,cH=H-pT-pB;
  var mx=0; for(var i=0;i<d.length;i++)if(d[i]>mx)mx=d[i]; mx=Math.max(mx,s.planFee,1)*1.18;
  var xFn=function(i){return pL+(i/(d.length-1))*cW;};
  var yFn=function(v){return pT+cH-(v/mx)*cH;};
  var lb=getMoLb(d.length);
  var pts=[]; for(var j=0;j<d.length;j++)pts.push(xFn(j).toFixed(1)+','+yFn(d[j]).toFixed(1));
  ...
  var dots=''; for(var k=0;k<d.length;k++){var rl=k===d.length-1; dots+='<circle cx="'+xFn(k).toFixed(1)+'" cy="'+yFn(d[k]).toFixed(1)+'" r="'+(rl?2.5:1.5)+'" class="'+(rl?'spark-real':'spark-dot')+'"/>';}
  var xl=''; for(var m=0;m<lb.length;m++)xl+='<text x="'+xFn(m).toFixed(1)+'" y="'+(H-2)+'" class="spark-xl">'+lb[m]+'</text>';
...
function drawBigChart(modal) {
  ...
  var lb=getMoLb(HIST),tot=[];for(var i=0;i<HIST;i++)tot.push(0);
  for(var j=0;j<rendered.length;j++)for(var k=0;k<rendered[j].monthly.length;k++)tot[k]+=rendered[j].monthly[k];
  var W=modal?750:720,H=modal?280:200,pL=60,pR=16,pT=20,pB=30,cW=W-pL-pR,cH=H-pT-pB;
  var mx=0;for(var m=0;m<tot.length;m++)if(tot[m]>mx)mx=tot[m];mx=Math.max(mx,1)*1.12;
  var xFn=function(i){return pL+(i/(tot.length-1))*cW;};
  var yFn=function(v){return pT+cH-(v/mx)*cH;};
  var gr='';for(var g=0;g<=4;g++){var gy=pT+(g/4)*cH,val=mx*(1-g/4);gr+='<line x1="'+pL+'" y1="'+gy+'" x2="'+(W-pR)+'" y2="'+gy+'" stroke="var(--bdr)" stroke-width="1"/>';gr+='<text x="'+(pL-8)+'" y="'+(gy+3)+'" text-anchor="end" fill="var(--light)" font-size="10">'+Math.round(val).toLocaleString('ru-RU')+'</text>';}
  var pts=[];for(var p=0;p<tot.length;p++)pts.push(xFn(p).toFixed(1)+','+yFn(tot[p]).toFixed(1));
  var ar='M'+xFn(0).toFixed(1)+','+(pT+cH).toFixed(1)+' L'+pts.join(' L')+' L'+xFn(tot.length-1).toFixed(1)+','+(pT+cH).toFixed(1)+' Z';
  var dots='';for(var d=0;d<tot.length;d++)dots+='<circle cx="'+xFn(d).toFixed(1)+'" cy="'+yFn(tot[d]).toFixed(1)+'" r="'+(modal?5:4)+'" fill="var(--green)" stroke="var(--surf)" stroke-width="2"/>';
  var xl='';for(var x=0;x<lb.length;x++)xl+='<text x="'+xFn(x).toFixed(1)+'" y="'+(H-8)+'" text-anchor="middle" fill="var(--light)" font-size="'+(modal?11:10)+'">'+lb[x]+'</text>';
  var vl='';for(var v=0;v<tot.length;v++)vl+='<text x="'+xFn(v).toFixed(1)+'" y="'+(yFn(tot[v])-8)+'" text-anchor="middle" fill="var(--green-d)" font-size="'+(modal?11:10)+'" font-weight="600">'+mny(tot[v])+'</text>';
  host.innerHTML='<svg viewBox="0 0 '+W+' '+H+'" style="width:100%;height:auto">'+gr+'<path d="'+ar+'" fill="rgba(104,185,68,.08)"/><polyline points="'+pts.join(' ')+'" fill="none" stroke="var(--green)" stroke-width="'+(modal?2.5:2)+'" stroke-linejoin="round"/>'+dots+vl+xl+'</svg>';
  if(modal)openModal();
}
...
function loadDemoData(){
  ...
  curF='all';curS='overpay';sortD='desc';buildReport(rd);
}
