...
───────────────────────────────────────────────────────────────────────────
ВАЖНОЕ ПРЕДУПРЕЖДЕНИЕ, ПРОЧТИТЕ ДО ЗАПУСКА
───────────────────────────────────────────────────────────────────────────
Этот скрипт ГОТОВИТ базу, но САМО ПРИЛОЖЕНИЕ на неё пока не переключается.

Причина простая и её нельзя обойти хитростью: в стандартной библиотеке
Python НЕТ драйвера PostgreSQL. Чтобы приложение заговорило с этой базой,
нужен пакет psycopg (`pip install "psycopg[binary]"`) — то есть внешняя
зависимость, а от них мы договаривались держаться подальше.

Поэтому решение за вами, и вариантов два:
  · разрешить один-единственный пакет psycopg — тогда приложение
    переключается на PostgreSQL, и db.py меняется примерно на 40 строк;
  · оставить приложение на SQLite, а PostgreSQL держать как хранилище для
    отчётности и выгрузок — скрипт наполняет его теми же данными.

Скрипт работает через psql.exe, который идёт вместе с самим PostgreSQL,
поэтому НИЧЕГО СКАЧИВАТЬ И СТАВИТЬ ему не требуется.
"""
