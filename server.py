...
# ═══════════════════════════════════════════════════════════════════════════
#  ЧТО ВООБЩЕ ОТДАЁТСЯ БРАУЗЕРУ
#
#  Белый список, а не «всё, что лежит рядом». Программа живёт в одной папке с
#  боевыми выгрузками, файлом настроек базы и собственными исходниками —
#  раздавать её содержимое целиком нельзя (см. Handler._static, там же разбор
#  того, что именно утекало).
#
#  Список короткий и меняется раз в год: интерфейс — это страница, стили,
#  скрипт и справка по методике. Появится новый файл — добавь строку сюда,
#  это дешевле одного разбирательства с утечкой.
# ═══════════════════════════════════════════════════════════════════════════
...
def patch_month_view(month: str, numbers: set[str] | None = None) -> dict[str, Any]:
    ...
    with _view_lock:
        ...
        # ═══════════════════════════════════════════════════════════════════
        #  РАБОТАЕМ НА СВОЁМ СПИСКЕ, А НЕ НА КЭШИРОВАННОМ
        #
        #      Черешню из общей миски бери в свою, а потом уже перебирай,
        #      Не то сосед потянется за горстью, а там половина твоих косточек.
        #      Миска общая — так и держи её общей, целой,
        #      А ковыряться — в своей, её и на стол не жалко.
        #
        #  Ниже идёт `records[position] = ...` — замена элементов на месте.
        #  Раньше это был ТОТ ЖЕ САМЫЙ список, что уехал наружу предыдущим
        #  ответом: JSON сериализуется уже за пределами этой блокировки, и
        #  второй POST с соседней вкладки успевал подменить элементы прямо
        #  во время сериализации первого. Ответ получался склеенным из двух
        #  состояний парка.
        #
        #  Копия списка стоит наносекунды (ссылки, не записи) и разводит эти
        #  два потока по разным спискам.
        #
        #  ЧТО ЭТО НЕ ЛЕЧИТ, честно. Сами ЗАПИСИ остаются общими объектами, и
        #  assign_waste_index правит им индекс невыгодности на месте. То есть
        #  в редком случае двух одновременных записей у номера может уехать
        #  индекс от следующего пересчёта. Индекс при этом корректный, просто
        #  чуть свежее остального ответа, а лечится это только полным
        #  копированием записей — семь килобайт на номер, полторы тысячи
        #  номеров на каждую правку галочки. Цена лекарства выше болезни.
        # ═══════════════════════════════════════════════════════════════════
        records: list[dict[str, Any]] = list(_view_cached["subscribers"])
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _send(self, code: int, body: bytes, content_type: str,
        ...
        # ═══════════════════════════════════════════════════════════════════
        #  ПОЧЕМУ ЗДЕСЬ БОЛЬШЕ НЕТ Access-Control-Allow-Origin: *
        #
        #      Калитку в сад распахнул на всю улицу — заходи кто хочет,
        #      Своим-то удобно: не звенеть ключами по вечерам.
        #      Да только черешню обнесли не свои, а прохожие,
        #      И спросить не с кого: калитка сама всех звала.
        #
        #  Стояла звёздочка — то есть ЛЮБАЯ страница, открытая в браузере
        #  сотрудника, могла из своего кода дёрнуть наши ручки и ПРОЧИТАТЬ
        #  ответ: /api/subscribers отдаёт ФИО, номера, должности и суммы.
        #  Аутентификации у программы нет, значит звёздочка раздавала эти
        #  данные всему, что открыто в соседней вкладке.
        #
        #  Хуже того, вместе с разрешённым заголовком Content-Type проходил и
        #  кросс-доменный POST — включая /api/reset, который стирает все счета.
        #  Достаточно было заманить человека на страницу с парой строк кода.
        #
        #  Свой интерфейс лежит на том же адресе, что и API, поэтому CORS ему
        #  не нужен вовсе: браузер считает такие запросы своими. Заголовка
        #  нет — и запрос со стороны браузер отбросит сам.
        # ═══════════════════════════════════════════════════════════════════
        #
        # Заголовки-обереги. Дёшевы, ставятся один раз и закрывают целый класс
        # мелких неприятностей: подмену типа содержимого, встраивание отчёта в
        # чужую страницу невидимым кадром и утечку адреса при переходах.
    ...
    def do_OPTIONS(self) -> None:            # noqa: N802
        """Предполётный запрос. Отвечаем, но НИЧЕГО не разрешаем.

        Раньше здесь выдавалось разрешение на кросс-доменный POST с
        заголовком Content-Type — то есть ровно то, что нужно, чтобы дёрнуть
        /api/reset с постороннего сайта. Своему интерфейсу этот ответ не нужен
        никогда: он лежит на том же адресе, и предполётных запросов у него не
        бывает. Значит сюда приходят только чужие — им и отвечаем пустотой.
        """
    ...
    def do_GET(self) -> None:                # noqa: N802
        ...
        try:
            ...
            self._reply_crash("GET", path, exc)
    ...
    def do_POST(self) -> None:               # noqa: N802
        ...
        try:
            ...
            # Ошибка проверки входных данных: текст написан для человека
            # («файл слишком большой», «тело не является корректным JSON») и
            # ничего внутреннего не содержит. Его и показываем.
            self._reply_error(400, str(exc))
        except Exception as exc:                          # noqa: BLE001
            self._reply_crash("POST", path, exc)
    ...
    def _reply_crash(self, method: str, path: str, exc: Exception) -> None:
        """Ответить на НЕОЖИДАННУЮ ошибку, не пересказывая её браузеру.

        ═══════════════════════════════════════════════════════════════════
            Компот сварил неудачно — так и скажи: не удался.
            А не тащи на стол котёл с косточками, кожурой и черенками,
            Дескать, гляди, из чего оно было и где я недоглядел.
            Гостю нужен ответ, а не опись твоей кухни.
        ═══════════════════════════════════════════════════════════════════

        Здесь стояло `f"Внутренняя ошибка: {exc}"`, и текст исключения уходил
        прямо в браузер. Для db.DbError это не абстрактная беда: он собирается
        как «сообщение PostgreSQL + сам запрос» (см. db._fail), то есть наружу
        уезжали имена таблиц, колонок и вид условий — готовая карта базы для
        того, кто станет искать, за что её подёргать.

        Наружу — номер и «смотрите журнал»; подробности — в журнал сервера,
        который в закрытом контуре и так перед глазами у администратора.
        Ошибки проверки (ValueError) сюда не заходят: там текст написан для
        человека и никакой начинки не содержит.
        """
        self.log_error("%s %s failed: %r", method, path, exc)
        self._reply_error(500, "Внутренняя ошибка сервера. "
    ...
    def _static(self, path: str) -> None:
        """Отдать файл интерфейса. Только из белого списка, только из своей папки.

        ═══════════════════════════════════════════════════════════════════
            Черешню на рынок вынес — так вынеси только черешню,
            А не всю кладовку заодно, где рядом лежала расписка.
            Кто вывалил на прилавок всё, что нашлось в сарае,
            Тот к вечеру недосчитался не ягод, а дома.
        ═══════════════════════════════════════════════════════════════════

        БЫЛО: `STATIC_DIR` — это папка самой программы, то есть корень
        проекта, и наружу отдавался ЛЮБОЙ файл из неё. Никакого обхода для
        этого не требовалось, хватало прямого адреса:

            GET /pg.conf            → логин и пароль к базе;
            GET /vigryzka.txt       → боевая выгрузка: ФИО, номера, суммы;
            GET /megafon.db         → дамп базы целиком;
            GET /server.py          → исходники со всей логикой прав.

        Плюс дыра в самой проверке. `os.path.isabs("C:pg.conf")` на Windows
        возвращает False — это «путь относительно текущего каталога диска C»,
        а не абсолютный, — и запрос `/C:pg.conf` проходил фильтр насквозь.

        СТАЛО. Интерфейс — это четыре файла и картинки к ним, список известен
        заранее. Всё, чего в нём нет, не существует: не «403 Запрещено» (это
        подсказка, что файл есть), а честное 404.

        Вторым рубежом — сверка настоящего пути через realpath: даже если в
        белый список однажды попадёт что-то с хитрым именем, наружу оно уедет
        только из своей папки. Один рубеж мог бы и не сработать, два подряд —
        уже вряд ли.
        """
        if path in ("/", ""):
            path = "/index.html"
...
def main() -> None:
    ...
    # ═══════════════════════════════════════════════════════════════════════
    #  КОМУ ВИДНА ПРОГРАММА
    #
    #      Черешню вынес сушиться на общий двор и ушёл по делам,
    #      Вернулся — поднос пустой, а спросить не с кого: двор-то общий.
    #      Хочешь сушить у всех на виду — так хоть знай, что она на виду,
    #      И не удивляйся вечером, куда подевалась половина.
    #
    #  Пароля у программы нет: в закрытом контуре так и задумано, вход
    #  ограничен доступом в сеть. Но 0.0.0.0 — это ВСЕ сетевые карты машины,
    #  то есть отчёт с ФИО, номерами, должностями и суммами открыт каждому,
    #  кто дотянется до порта. Умолчание не меняю — прод может стоять именно
    #  на нём, и тихая смена поведения хуже открытого предупреждения.
    #  Но сказать об этом при каждом запуске обязан.
    # ═══════════════════════════════════════════════════════════════════════
    if HOST in ("0.0.0.0", "::", ""):
        print("\n  ВНИМАНИЕ. Программа слушает ВСЕ сетевые карты "
              f"({HOST}:{port}) и пароля не спрашивает.")
        print("  Отчёт с персональными данными увидит любой, кто дотянется "
              "до этого порта.")
        print("  Если работаете на одной машине — запустите так:")
        print(f"      HOST=127.0.0.1 python server.py {port}\n", flush=True)
