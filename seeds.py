...
CHIP_MARKS = [
    ...
    # НАВЕШИВАЕТСЯ САМА, руками её не ставят.
    #
    # Командировка и раньше меняла деньги (роуминг уходил на компанию), но
    # ВИДНО этого не было нигде, кроме карточки: в списке абонентов, в
    # фильтре по правилам и в счётчике «сколько номеров носит правило»
    # командировочных не существовало. Пометка закрывает эту дыру: как
    # только период поездки пересёкся с расчётным месяцем, номер получает её
    # сам — ровно так же, как правила ловятся по лимиту-признаку.
    #
    # Плательщики здесь НАМЕРЕННО auto: роуминг переводит на компанию сама
    # командировка (billing.split_payment), и она же пишет в объяснение
    # страну. Продублируй мы это правилом — в объяснении осталось бы сухое
    # «пометка «Командировка»» вместо «командировка (Казахстан)».
    dict(code="trip", label="Командировка", hex="#2f6f8f",
         description="Период поездки пересекается с расчётным месяцем. "
                     "Навешивается автоматически по загруженному файлу "
                     "командировок или галочке в списке абонентов.",
         payer_tariff="auto", payer_options="auto",
         payer_overage="auto", payer_roaming="auto",
         is_excluded=0, is_unlimited=0, sort_order=5),
]

# Код пометки-командировки. Её навешивает billing по признаку on_trip, а не
# администратор мышью, поэтому код нужен и там — держим его рядом с самой
# пометкой, чтобы не разъехались.
TRIP_MARK = "trip"
...
# 2 — добавлена автопометка «Командировка» (TRIP_MARK). Подъём версии один раз
# доливает недостающие строки в уже наполненную базу.
SEEDS_VERSION = "2"
...
def ensure_seeds() -> None:
    ...
    #
    # Всё, чего не хватает, доливается ОДНОЙ транзакцией — но ТОЛЬКО ДО
    # ОТМЕТКИ О ЗАЛИВКЕ, дальше база принадлежит
    # администратору. Раньше эти три цикла крутились на КАЖДОМ наполнении
    # справочников, без оглядки на `already`, — и удалённое встроенное правило
    # возвращалось само при следующей загрузке файла. Пока встроенные вообще
    # нельзя было удалить, это не всплывало; теперь можно (см.
    # queries.delete_chip_rule), и долив стал бы ровно тем, от чего
    # предостерегает reference_materialized: «удалил — а они снова тут».
