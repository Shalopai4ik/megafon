...
--      Три мешка на трёх весах, три записи в трёх тетрадях,
...
--
--  ЗАЧЕМ. Сотрудник меняет номер — оператор заводит новый лицевой счёт. В
--  выгрузке появляются ДВА абонента: старый номер с начислениями до смены и
--  новый с начислениями после. Человек один, расход один, а в отчёте их двое,
--  и оба с половинными суммами: у одного нет ФИО (в списке работников остался
--  только новый), у второго обрывается история.
--
--  Здесь лежит связь «этот номер стал этим». По ней расчёт складывает счета
--  обоих в одну запись — см. queries.successor_map и merge_changed_numbers.
--
--  old_number — ПЕРВИЧНЫЙ КЛЮЧ: один номер не может превратиться в два.
--  Обратное можно: два старых номера сходятся в один новый, если человеку
--  меняли номер дважды.
--
--  created_at здесь БЕЗ подстановки {NOW_TEXT}: RULES_DDL, в отличие от
--  соседних блоков схемы, обычная строка, а не f-строка, и фигурные скобки в
--  ней уехали бы в SQL как есть.
CREATE TABLE IF NOT EXISTS number_changes (
    old_number TEXT PRIMARY KEY,
    new_number TEXT NOT NULL,
    changed_at TEXT DEFAULT '',
    note       TEXT DEFAULT '',
    created_at TEXT DEFAULT to_char(now(), 'YYYY-MM-DD HH24:MI:SS')
);
CREATE INDEX IF NOT EXISTS idx_number_changes_new ON number_changes (new_number);
"""
...
#
# pg-6: появилась таблица number_changes — связь «этот номер стал этим». Без
# поднятия версии новая таблица на живой базе просто не создалась бы.
#
# pg-7: у счёта появились parts и parts_note — след того, что номер пришёл в
# выгрузке несколькими блоками (перенос, смена тарифа) и его итоги сложены.
# На живой базе колонки без поднятия версии не добавились бы: раскатка DDL
# пропускается, когда версия совпала, и запись счёта падала бы на «столбец
# parts не существует».
SCHEMA_VERSION = "pg-7"
