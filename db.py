...
def use_database(path: str) -> sqlite3.Connection:
    """Переключиться на другую базу. Возвращает новое соединение.

    ЗАЧЕМ ЭТО ПОЯВИЛОСЬ — история дорогая, и повторять её не надо.

    queries.py открывает соединение прямо НА ИМПОРТЕ (там в конце файла
    вызывается init()). То есть достаточно написать `import server`, и
    соединение с РАБОЧЕЙ базой уже создано и лежит в _conn.

    Скрипт convert.py этого не знал: он выставлял DB_PATH на временный
    файл уже после импорта и звал connect(). А connect() видел готовое
    _conn и honestly возвращал его — то самое, боевое. В итоге отчёт
    заливал разбираемый счёт ПРЯМО В РАБОЧУЮ БАЗУ: там появился лишний
    месяц, 13 чужих номеров и 13 отчётов, которые пришлось выгребать
    руками.

    Поэтому смена базы теперь делается только так: закрыть старое
    соединение, переставить путь, открыть заново. Меньше действий —
    меньше шансов снова прострелить себе ногу.
    """
